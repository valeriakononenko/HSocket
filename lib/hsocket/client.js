...
hsocket.Client.prototype.__findModify =
  ...
  if (this.__index instanceof hsocket.Index) {
    ...
    var args = [this.__index.getId(), operationType, columns, limit,
          inCondition, filters, mod];

    this.__writeChannel.send(hsocket.encodeCommand(args),
...
hsocket.Client.prototype.find =
  ...
  if (this.__index instanceof hsocket.Index) {
    ...
    var command = [this.__index.getId(), operationType, values.length,
                   values, limit, inCondition, filters];
...
hsocket.Client.prototype.insert = function(values, complete, cancel) {
  if (this.__index instanceof hsocket.Index) {
    if (values.length === this.__index.getColumns().length) {
      ...
      cancel('(hsocket) Wrong number of columns in index for INSERT.');
    }
  } else {
    cancel('(hsocket) Index is not set.');
  }
