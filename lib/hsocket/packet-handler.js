


/**
 * @param {!Function} complete Result handler.
 * @param {function(string, number=)} cancel Error handler.
 * @param {hsocket.RequestType=} opt_requestType
 * @param {number=} opt_terminal
 * @param {number=} opt_separator
 * @constructor
 */
hsocket.PacketHandler = function(complete, cancel,
                                 opt_requestType, opt_terminal, opt_separator) {
  var self = this;

  /**
   * @type {number}
   */
  this.__separator = opt_separator || 0x09;

  /**
   * @type {number}
   */
  this.__terminal = opt_terminal || 0x0a;

  /**
   * @type {!hsocket.Data}
   */
  this.__data = new hsocket.Data(this.__terminal, this.__separator);

  /**
   * @type {boolean}
   */
  this.__isComplete = false;

  /**
   *
   */
  this.__dataHandler = function() {
    var code = self.__data.getHeader();
    var requestType = opt_requestType || hsocket.RequestType.EMPTY;
    if (code !== 0) {
      cancel(self.__data.getValue(hsocket.RequestType.ARRAY)[0], code);
    } else {
      complete(self.__data.getValue(requestType));
    }
  };
};


/**
 * Returns true if a packet was handled.
 *
 * @param {hsocket.Cursor} cursor object.
 * @param {!Buffer} chunk Data packet.
 * @return {boolean} Flag of packet handling.
 */
hsocket.PacketHandler.prototype.process = function(cursor, chunk) {
  this.__data.process(cursor, chunk);

  if (this.__data.isComplete()) {
    this.__isComplete = true;
    process.nextTick(this.__dataHandler);
  } else {
    cursor.breakParsing();
  }

  return this.__isComplete;
};


/**
 * Clears a packet for reconnect.
 */
hsocket.PacketHandler.prototype.reset = function() {
  this.__data = new hsocket.Data(this.__terminal, this.__separator);
};
