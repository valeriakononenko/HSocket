...
hsocket.PacketHandler = function(complete, cancel,
                                 opt_responseType, opt_terminal, opt_separator) {
  var self = this;
  ...
  this.__dataHandler = function() {
    ...
    var responseType = opt_responseType || hsocket.ResponseType.EMPTY;
    if (code !== 0) {
      cancel(self.__data.getValue(hsocket.ResponseType.ARRAY)[0], code);
    } else {
      complete(self.__data.getValue(responseType));
    }
  };
};
 ...
 *
 * @param {hsocket.Cursor} cursor object.
